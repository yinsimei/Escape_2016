...
public class physicWalk_MouseLook : MonoBehaviour {
	...
	void FixedUpdate ()
	{
        ...
        float fDeplacementX = Input.GetAxis("Mouse X");
        float fDeplacementY = Input.GetAxis("Mouse Y");

        //Smoothed stuff
        smoothedMouse = Vector2.Lerp( smoothedMouse, new Vector2(fDeplacementX, fDeplacementY), 1f/smoothing );
